...
# Format the previous month 
    ...
    formatted_month = previous_month.replace(day=1).strftime("%Y-%m-%d 00:00:00+00:00")
...
query="""select * from IBPExtractParametersV3 where Frequency in ("Daily","Weekly") and tableType="FACT" and TargetName="CAS_GBQ_DP_ZWKPRODCUSTSOLDTOSAREA_2_d";"""
...
def create_subdag(parent_dag_id,child_dag_id,args,TargetName,dataframe):
    ...
    with subdag:
        ...
        PeriodExtractGen=ExtractIbpDataGen(entityset,columnList=['PERIODID3_TSTAMP'])

        DfPeriods=next(PeriodExtractGen)
        filtredDf= DfPeriods[DfPeriods['PERIODID3_TSTAMP'].apply(lambda x: x == formatted_month)]
        ...
        for elem in PeriodValues:
            ...
            if Filter is not None:
                Filter+=" and PERIODID3_TSTAMP" + " eq " + "datetime'" + f"{elem}" + "' "
            else:
                Filter= " PERIODID3_TSTAMP eq " + "datetime'" + f"{elem}" + "'"
            ...
            execute_task=PythonOperator(
                ...
                'filename' : f"PAST_{filename}_{indice}"
                }
                )
            Filter=VARIABLES.Default_values.get('Filter') if not group_df['Filter'].iloc[0] else group_df['Filter'].iloc[0]
...
with DAG(
    dag_id='IBP_DailyWeekly_Past_DAG',
    ...
    for TargetName in df['TargetName'].unique():
        ...
        subdag=create_subdag('IBP_DailyWeekly_Past_DAG',f'extracting_{taskname}' , {'schedule_interval': None, 'start_date': datetime(2023, 5, 20)},TargetName,dataframe=df),
    dag=dag
    )
